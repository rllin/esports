...
for s in subjects:
    if not s.bio['rank'].isdigit():
        ...
        all_ikis = list(itertools.chain(*s.section_ikis))
        avg_time.append(np.median([reduce(lambda x,y: x+y, e) for e in all_ikis]))
...
for k, gp in data.groupby('exp'):
    ...
    sns.plt.savefig('correct execution exp: ' + str(k) + '.pdf')
